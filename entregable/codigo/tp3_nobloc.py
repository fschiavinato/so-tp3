		...
		while(len(queue) > 0 or cant > 0):
			for node in queue:
				...
				if(node_rank not in processed):
					...
					cant +=1
			response = self.__comm.recv(source=MPI.ANY_SOURCE, tag=TAG_NODE_FIND_NODES_RESP)
			cant -=1
	...
	def __find_nodes_join(self, contact_nodes):
		...
		while(len(queue) > 0 or cant > 0):
			...
			for node in queue:
				node_hash, node_rank = queue.pop()
				if node_rank not in processed:
					#pendientes.append(node_rank)
					processed.add(node_rank)
					cant+=1
					self.__comm.isend((self.__hash, self.__rank), dest=node_rank, tag=TAG_NODE_FIND_NODES_JOIN_REQ)
			response = self.__comm.recv(source=MPI.ANY_SOURCE, tag=TAG_NODE_FIND_NODES_JOIN_RESP)
			cant -= 1
